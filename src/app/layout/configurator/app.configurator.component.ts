          ...
          <div class="color-buttons">
            @for (primaryColor of primaryColors(); track primaryColor) {
              <button
                ...
                [ngClass]="{
                  'active-color': primaryColor.name === selectedPrimaryColor(),
                }"
                [style]="{
                  'background-color':
          ...
          <div class="color-buttons">
            @for (surface of surfaces; track surface) {
              <button
                ...
                [ngClass]="{
                  'active-color': surface.name === selectedSurfaceColor(),
                }"
                [style]="{
                  'background-color':
                    surface.name === 'noir'
...
export class AppConfiguratorComponent implements OnInit {
    ...
    try {
      ...
      if (storedConfig) {
        ...
        this.configService.appState.update((state) => ({
          ...
          darkTheme:
            config.darkTheme !== undefined ? config.darkTheme : state.darkTheme,
        }));
  ...
  primaryColors = computed(() => {
    ...
    const presetPalette: Record<string, Record<string, string>> = presets[
      presetKey
    ]?.primitive || {};
  ...
  onPresetChange(event: any, saveToStorage: boolean = true) {
    ...
    type PresetKeys = keyof typeof presets;
    const preset =
      (event in presets ? presets[event as PresetKeys] : undefined) ||
      'default value';
