...
export class NotificationComponent implements OnInit, OnDestroy {
  ...
  /**
   * Fetches all Notification with given parameters
   * @param params - Parameters to filter Notification
   */
  findAllNotification(): void {
    ...
    this.notificationService
      ...
      .pipe(
        filter((res: HttpResponse<NotificationResponse>) => res.ok),
        map((res: HttpResponse<NotificationResponse>) => res.body),
        takeUntil(this.destroyed$),
      )
      .subscribe({
        next: (res: NotificationResponse | null) => {
          if (res != null) {
        ...
        error: (res: HttpErrorResponse) => {
          this.messageService.add({
            severity: 'error',
            summary: 'Failed',
            detail: `Failed To Load all Notification.`,
            life: 6000,
          });
          this.isDataLoading = false;
